...
export class ActivityResolver {
  ...
  @Query(() => [ActivityDto])
  @UseGuards(AuthGuard)
  async getUserFavoritesActivities(
    @Context() context: any,
  ): Promise<ActivityDto[]> {
    const activities = await this.activityService.findUserFavoritesActivities(
      context.user!.id,
    );
    return activities.map((activity) => this.activityMapper.convert(activity));
  }
  ...
  async toggleFavoriteActivity(
    //TODO: destructure user context
    @Context('user') { id: userId }: any,
    @Args('activityId') activityId: string,
  ): Promise<ActivityDto> {
    const toggleFavoriteActivity =
      await this.activityService.toggleFavoriteActivity(userId, activityId);
    return this.activityMapper.convert(toggleFavoriteActivity);
  }
}
