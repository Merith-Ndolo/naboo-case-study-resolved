...
export default function ActivityForm() {
  ...
  const { refetch: refreshMyActivities } = useQuery(GetUserActivities);
  const {  refetch: refreshActivities } = useQuery(GetActivities);
  ...
  //TODO: Best nomination instead of data use cities
  useEffect(() => {
    if (debouncedSearch) {
      searchCity(debouncedSearch)
        .then((cities) => {
          setDisplayedCities(cities.map((city) => ({ value: city.nom, label: city.nom })));
        })
        .catch((err) => {
